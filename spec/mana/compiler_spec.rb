...
RSpec.describe Mana::Compiler do
  ...
  describe "cache hit / miss" do
    ...
    it "skips cache when prompt is nil (IRB/eval environment)" do
      klass = Class.new do
        def irb_method(n)
          ~"some prompt"
        end
      ...
      generated_code = "def irb_method(n)\n  n + 1\nend"
      allow(described_class).to receive(:generate).and_return(generated_code)

      # Even if cache file exists, should NOT use it (prompt_hash is nil)
      ...
      FileUtils.mkdir_p(cache_dir)
      File.write(cache_path, "# Auto-generated by ruby-mana | prompt_hash: whatever\n\ndef irb_method(n)\n  n * 99\nend\n")

      described_class.compile(klass, :irb_method)

      obj = klass.new
      expect(obj.irb_method(5)).to eq(6)  # uses generate result (n+1), not cached (n*99)
      expect(described_class).to have_received(:generate)
    end
  end
