...
RSpec.describe "Nested prompts" do
  ...
  describe "memory isolation" do
    it "inner call gets fresh short-term memory" do
      outer_memory = Mana::Context.current
      outer_memory.messages << { role: "user", content: "outer context" }
      outer_memory.messages << { role: "assistant", content: [{ type: "text", text: "outer reply" }] }
    ...
    # Long-term memory sharing test moved to claw specs
    # (long-term memory is now owned by claw, not mana)

    it "restores outer memory after nested call completes" do
      outer_memory = Mana::Context.current
      outer_memory.messages << { role: "user", content: "outer message" }
      ...
      expect(Mana::Context.current).to equal(outer_memory)
      # Outer short-term should still contain its original messages (plus new ones from the outer call)
      outer_contents = outer_memory.messages.select { |m| m[:role] == "user" && m[:content].is_a?(String) }
        .map { |m| m[:content] }
  ...
  # incognito nesting tests removed — incognito now handled by claw
